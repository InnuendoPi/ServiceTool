...
3. **Migration starten** wählen. USB und Stromversorgung bis zum Abschluss
   verbunden lassen. Der Vorgang dauert ungefähr acht Minuten; der aktuelle
   Arbeitsschritt wird angezeigt.
...
`firmware.bin` und `serviceapp.bin` sowie die passenden Webdateien und
Versionsinformationen. Eine zusätzliche `migration.json` ist nicht erforderlich.
