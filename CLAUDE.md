...
- **Everything backend-side lives in one file, `app.py`** (~5400 lines, ~190 top-level functions, ~8 classes). There is no package/module split — when making changes, search within this single file rather than expecting a multi-module layout.
...
- **Telegraf integration**: ServiceTool only generates a temporary Telegraf config and starts/stops the process; Telegraf itself is downloaded on first use (not bundled) into `cache/tools`. Destination passwords are not persisted to `config.json` unless the user explicitly opts in.
